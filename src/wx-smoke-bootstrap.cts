...
(function bootstrapSmoke(): void {
  const smokeState: any = {
  ...
  function getSummaryColor(): string {
  ...
  function updateSummary(): void {
    ...
    const hasFailure =
      canvasState === 'failed' ||
      canvasState === 'invalid' ||
      canvasState === 'no-2d-context' ||
      readFileState === 'failed';
  ...
  function drawTextLine(text: string, x: number, y: number, color?: string, font?: string): void {
    if (!smokeState.ctx) {
      return;
    }
  ...
  function captureSystemInfo(): void {
  ...
  function setupTouch(): void {
    ...
    if (typeof wx.onTouchStart === 'function') {
      wx.onTouchStart(function onTouchStart(): void {
    ...
    if (typeof wx.onTouchMove === 'function') {
      wx.onTouchMove(function onTouchMove(): void {
    ...
    if (typeof wx.onTouchEnd === 'function') {
      wx.onTouchEnd(function onTouchEnd(): void {
  ...
  function setupReadFileSmoke(): void {
  ...
  if (typeof globalThis !== 'undefined') {
    (globalThis as any).__MEMBRANE_WX_SMOKE__ = smokeState;
  }
